...
export default function ContainerInfo5() {
  return (
    <div className="grid grid-rows-[10%_35%_45%_10%] sm:grid-rows-[10%_20%_50%_10%] lg:grid-rows-[15%_26%_42%_12%] xl:grid-rows-[15%_26%_47%_12%] w-full min-h-[600px] xl:min-h-[400px] gap-5">
      <span className="relative flex row-start-1 text-[30px] md:text-3xl lg:text-4xl font-bold
      leading-[135%] tracking-[0.02em] text-[#333333] self-stretch text-nowrap">Партнерам</span>
      <span className="relative block row-start-2 font-normal text-xs lg:text-sm leading-[150%] text-[#454545]">Более 18 лет продукция компании «Алекс-мебель» уверенно завоевывает мебельный рынок России. Высокое качество, современный дизайн, безупречная функциональность обеспечивают неизменно высокий спрос на все позиции ассортимента. Не последним в деле расширения оборотов производства является четкая 
      ...
      <div className="row-start-3 grid grid-rows-6 md:grid-cols-[45%_55%] md:grid-rows-3 lg:grid-cols-3 lg:grid-rows-2 gap-4 text-xs lg:text-sm">
      ...
      <button className="
        flex flex-row items-center justify-center p-[16px_24px] gap-2 row-start-4
        w-[180px] lg:w-[250px] max-h-[40px] left-[50px] top-[555px] bg-[#F3BC2B] rounded-[5px] text-white text-sm text-nowrap
      ">Подробнее</button>
    </div>
  );
};